...
if (!$mysqli->connect_errno) {
    ...
    $current_org='';
    while ($org = $res_organizations->fetch_assoc()){
        $organizations[] = $org['name'];
    }

    foreach ($ad_departments as &$dept) {
		if (!in_array($dept, $organizations)) {
			$dept = $mysqli->real_escape_string($dept);
			$res_add_organization = $mysqli->query('INSERT INTO ost_organization (name,created,updated) VALUES ("' . $dept. '", NOW(),NOW())' );
		}
	}
    /*END jriggs org mapping*/
	...
	if ($config->agents == "true") {
		...
		while ($row_ostagents = $res_ostagents->fetch_assoc()) {
			...
			if (isset($array_ad_users[$row_ostagents['username']]) && $row_ostagents['username'] == $array_ad_users[$row_ostagents['username']]['samaccountname'][0]) {
                ...
                echo "DEPT:".$array_ad_users[$row_ostagents['username']]['department'][0]."<br>";	
	...
	$res_users = $mysqli->query($qry_ostusers);

	$osusers = array();
	while ($users = $res_users->fetch_assoc()){
		echo $functions->logg("Found user with name ".$users['username']);
		$osusers[] = $users['username'];
	}
	
	$rows_ostusers = $res_ostusers->fetch_assoc();
	// Find User's not in osTicket and add them
	foreach ($array_ad_users as &$ad_row) {		
		if (!in_array($ad_row['samaccountname'][0], $osusers)) {
			$username = $mysqli->real_escape_string($ad_row['samaccountname'][0]);
			$email = $mysqli->real_escape_string($ad_row['mail'][0]);
			$dispnm = $ad_row['personaltitle'][0]." ".$ad_row['sn'][0];
			$displayname = $mysqli->real_escape_string($dispnm);

			$res_update_ldapuser1 = $mysqli->query('INSERT INTO ost_user (status,name,created,updated,default_email_id,org_id) VALUES ("0","' . $displayname.'",NOW(),NOW(),"0","0")' ) or die(mysqli_error($mysqli));
			$res_user = $mysqli->query("SELECT id FROM ost_user WHERE name='" . $displayname."'");
			$user_id = $res_user->fetch_assoc()['id'];

			$res_update_ldapuser = $mysqli->query('INSERT INTO ost_user_account (username,status,timezone,user_id) VALUES ("' . $username. '","1","America/New_York","'.$user_id.'")' ) or die(mysqli_error($mysqli));
			$res_update_ldapuser2 = $mysqli->query('INSERT INTO ost_user_email (address,user_id) VALUES ("' . $email. '","'.$user_id.'")' ) or die(mysqli_error($mysqli));
			$qry_link_tables = "update ost_user u
								set u.default_email_id = (select id from ost_user_email where address='" . $email."') 
								where u.name='".$displayname."' ";
			$res_link_table = $mysqli->query($qry_link_tables);

			echo $functions->logg("Added osTicket User: ".$ad_row['samaccountname'][0]."");
		}
	}
	...
	while ($row_ostusers = $res_ostusers->fetch_assoc()) {
        ...
        $qry_update_ostuser_dept='update ost_user u
                                    ...
                                    set u.org_id = (select id from ost_organization where name="'.$dept.'") 
		...
		if (isset($array_user_ldap_attribute) && $row_ostusers['username'] == $array_user_ldap_attribute['samaccountname']) {
			...
			// DEBUG and LOGG OsTicket User not in LDAP so delete
			echo $functions->logg("osTicket User: ".$row_ostusers['username']." not existing in LDAP. Deprovisioning This User");
			
			$username = $mysqli->real_escape_string($row_ostusers['username']);	

			$res_del_osuser = $mysqli->query('update ost_user_account
												INNER JOIN ost_user on ost_user.id = ost_user_account.user_id
												set ost_user_account.status=3,ost_user.org_id=0
												where ost_user_account.username="' . $username. '"' );
		}
